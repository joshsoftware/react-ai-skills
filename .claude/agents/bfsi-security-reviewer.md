...
- Form inputs / env not validated with Zod (form schema belongs in `utils.ts`; env in `env.ts`). Note: API _responses_ are typed via `types.ts` interfaces and are NOT runtime-validated by default — do not flag a missing response `.parse()`.
