...
If the data comes from `/api/...`, it belongs in TanStack Query. Mirroring creates two sources of truth and stale UI. The only exception is the _id_ of a fetched record stashed for cross-component reference — never the record itself.
...
Real BFSI use case: track idle activity for auto-logout on sensitive routes. The store holds the _state_; the idle timer from `@react-vault/core/auth` drives it.
