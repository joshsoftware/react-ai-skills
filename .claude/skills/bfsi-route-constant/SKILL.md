...
Promotes a UI route path to the centralised `ROUTES` map and updates every call-site that referenced the literal string. Frontend route paths must never live inline — `constants-organization` covers the _why_; this skill automates the _how_.
